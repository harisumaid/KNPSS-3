...
handler.post(async (req, res) => {
  ...
  const achievementOld = JSON.parse(await fetchForId(req.body._id));
  ...
  achievementOld[0].imagesPath.forEach(async (element) => {
  ...
  achievementOld[0].pdfsPath.forEach(async (element) => {
  ...
  // deleted the achievementOld (achievement) it was not a Mongoose Document anymore are playing with
  // that would not had been easy so created a new document with same _id after deleting achievementOld
